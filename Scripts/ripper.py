...
class DataFile:
    ...
    def __init__(self, filename: str):
        ...
        with open(filename, "rb") as f:
            ...
            logger.debug(f"Loaded data file into buffer: {self.data.size()} bytes")
    ...
    def read(self, indexId: int, archiveId: int, sector: int, size: int) -> bytearray:
        ...
        while size > readBytesCount:
            ...
            logger.debug(f"Reading sector {sector} at position {self.data.readerIndex}")
...
class IndexFile:  # type: ignore # noqa
    ...
    def __init__(self, filename: str):
        ...
        with open(filename, "rb") as f:
            ...
            logger.debug(f"Loaded new index into buffer: {self.buffer.size()} bytes")
...
class Container:
    ...
    def decompress(b: bytearray, keys: List[int] = []) -> Container:
        ...
        if compression == 0:
            ...
            decryptedData = Container.decrypt(encryptedData, len(encryptedData), keys)
...
class DiskStorage(Storage):  # type: ignore # noqa
    ...
    def __init__(self):
        ...
        self.indexFiles = []
        self.dataFile = DataFile(os.path.join(self.root, self.mainFileCacheDat))
        indexMasterFilename = f"{self.mainFileCacheIdx}255"
        self.indexFileMaster = IndexFile(os.path.join(self.root, indexMasterFilename))
    ...
    def loadIndex(self, index: Index) -> None:
        ...
        logger.success(f"Read index {IndexType(index.id).name}: {indexData.size()}")
        if indexData.size() == 0:
            return
